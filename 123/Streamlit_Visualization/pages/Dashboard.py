...
# Import AI service (after page config)
try:
...
# -------------------------
with tab5:
    st.subheader("🤖 AI Assistant - ChatGPT Integration")
    
    if not AI_AVAILABLE:
        st.warning("⚠️ AI Assistant requires OpenAI API key.")
        st.info("""
        **To enable AI Assistant:**
        ...
        3. Or create `.env` file with: `OPENAI_API_KEY=your_key`
        """)
        st.stop()
    ...
    if not ai_service.is_available():
        ...
        **Setup Instructions:**
        ...
        # Chat input
        user_input = st.chat_input("Ask me about cybersecurity, IT operations, or data analysis...")
        ...
        if user_input:
            ...
            with st.spinner("🤖 AI is thinking..."):
                ...
                ai_response = ai_service.chat(user_input, context)
        ...
        with col1:
            if st.button("🛡️ Get Security Advice"):
                if not inc_df.empty:
                    sample_incident = inc_df.iloc[0]
                    with st.spinner("Getting security advice..."):
                        advice = ai_service.get_security_advice(
                            sample_incident.get('title', 'Unknown'),
                            sample_incident.get('severity', 'Medium'),
                            f"Status: {sample_incident.get('status', 'Unknown')}"
                        )
                    st.info(advice)
        
        with col2:
            if st.button("📊 Analyze Data Trends"):
                summary = f"Total incidents: {len(inc_df)}, Datasets: {len(dat_df)}, Tickets: {len(tic_df)}"
                with st.spinner("Analyzing trends..."):
                    analysis = ai_service.analyze_data_trend("Intelligence Platform", summary)
                st.info(analysis)
